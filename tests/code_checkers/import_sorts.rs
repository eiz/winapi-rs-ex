...
/// Compare strings according to rustfmt style (snake_case < CamelCase < SNAKE_CASE)
pub(crate) fn compare_rustfmt(left: &str, right: &str) -> Ordering {
    if left.starts_with(char::is_uppercase) && right.starts_with(char::is_lowercase) {
        return Ordering::Greater;
    }
    if left.starts_with(char::is_lowercase) && right.starts_with(char::is_uppercase) {
        return Ordering::Less;
    }
    if is_upper_snake_case(left) && !is_upper_snake_case(right) {
        return Ordering::Greater;
    }
    if !is_upper_snake_case(left) && is_upper_snake_case(right) {
        return Ordering::Less;
...
fn check_inner_imports(
    ...
    if imports.len() > 1 {
        ...
        for pos in 0..imports.len() - 1 {
            ...
            if compare_rustfmt(&imports[pos], &imports[pos + 1]) == Ordering::Greater {
                writeln!(
                    &mut io::stderr(),
                    "[{}:{}] In \"{}\": \"{}\" should be after \"{}\"",
                    filename,
        ...
        if issues == true {
            ...
            sorted.sort_by(|a, b| compare_rustfmt(a, b));
            writeln!(
                &mut io::stderr(),
                "==> Correct imports: \"use {}{{{}}};\"",
...
fn check_import_sorting<P: AsRef<Path>>(p: P, errors: &mut u32) {
    let r_p = p.as_ref();
        ...
        if current_import.is_some() {
            ...
            if line.ends_with(";") || line.ends_with("}") {
                if let Some(ref current_import) = current_import {
                    ...
                    imports[len]
                        .1
                        .push(format!("{{{}}};", current_import.join(", ")));
                    {
                ...
                let new_entries: Vec<String> = line
                    .split(",")
    ...
    if imports.len() > 1 {
        for pos in 0..imports.len() - 1 {
            let mut i = 0;
            while i < imports[pos].1.len() - 1
                && i < imports[pos + 1].1.len() - 1
                && compare_rustfmt(&imports[pos].1[i], &imports[pos + 1].1[i]) == Ordering::Equal
            {
                i += 1;
            }
            ...
            if i >= imports[pos + 1].1.len() - 1
                || compare_rustfmt(&imports[pos].1[i], &imports[pos + 1].1[i]) == Ordering::Greater
            {
                writeln!(
                    &mut io::stderr(),
                    "[{}:{}] \"use {}\" should be after \"use {}\"",
                    s_path,
                    imports[pos].0,
                    imports[pos].1.join("::"),
                    imports[pos + 1].1.join("::")
                )
                .unwrap();
                *errors += 1;
            }
